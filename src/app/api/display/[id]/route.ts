...
async function getEventAlerts(eventId, headers, baseUrl) {
  try {
    ...
    const select = 'wdrgns_alertcolour,wdrgns_alertimage,wdrgns_alerttext,wdrgns_endtime,wdrgns_starttime,_wdrgns_event_value';
    ...
    console.error("Error fetching event alerts:", err.response?.data || err.message);
    return [];
  }
}
...
async function getEventSchedules(eventId, headers, baseUrl) {
  try {
    ...
    const futureUrl = `${baseUrl}/wdrgns_eventschedules?$filter=${encodeURIComponent(futureFilter)}&$select=${select}&$orderby=wdrgns_starttime asc`;
    ...
    const pastUrl = `${baseUrl}/wdrgns_eventschedules?$filter=${encodeURIComponent(pastFilter)}&$select=${select}&$orderby=wdrgns_starttime desc`;
    ...
    console.error("Error fetching event schedules:", err.response?.data || err.message);
    return [];
  }
}
...
async function getTicketLinks(ticketTypeId, headers, baseUrl) {
  try {
    ...
    console.error("Error fetching ticket links:", err.response?.data || err.message);
    return [];
  }
}
...
export async function GET(req, { params }) {
  ...
  let selectedEvent =
    events.find(e => new Date(e.wdrgns_startdate) <= now && new Date(e.wdrgns_enddate) >= now) ||
  ...
  const [promoter, location, promoterLogo, alerts, schedules, ticketLinks] = await Promise.all([
  ...
  const result = {
    ...
    ticketType: ticketType ? {
      ...
      enableAskAdam: ticketType.wdrgns_enableaskadam === 948090000
        ? "enabled"
